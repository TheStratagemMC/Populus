 ...
 */
public class AlliancesCommand extends SubExecutor{
    @Override
    ...
    public void execute(Player player, String[] arg) {
        ...
        if(arg.length == 0) {
            if(plugin.getPermissionsManager().has(player, "simpleclans.anyone.alliances")) {
                ...
                chatBlock.addRow(new String[]{"  " + headColor + plugin.getLang("clan"), plugin.getLang("allies")});
                Iterator more = clans.iterator();

                while(more.hasNext()) {
                    Clan clan = (Clan)more.next();
                    if(clan.isVerified()) {
                        chatBlock.addRow(new String[]{"  " + ChatColor.AQUA + clan.getName(), clan.getAllyString(ChatColor.DARK_GRAY + ", ")});
                    }
                }
            ...
            ChatBlock.sendMessage(player, ChatColor.RED + MessageFormat.format(plugin.getLang("usage.clan.alliances"), new Object[]{plugin.getSettingsManager().getCommandClan()}));
        }
